    ...
    for site in SITES:
        ...
        try:
            border_color = driver.find_element_by_id(
                "arrivaldate"
            ).value_of_css_property("border-top-color")
...
for i in range(NUM_RESERVATIONS):
    ...
    url_request = (
        "http://www.recreation.gov/campsiteDetails.do?siteId={site_id}&contractCode=NRSO&parkId={park_id}&arvdate="
        + ARV_DATE
        + "&lengthOfStay="
        + LENGTH_OF_STAY
    )
    ...
    if selected_site:
        ...
        if noerrors:
            ...
            username_field = WebDriverWait(driver, 5).until(
                EC.presence_of_element_located((By.CSS_SELECTOR, "#emailGroup input"))
            )
            ...
            if driver.find_element_by_id("equip").is_enabled():
                driver.find_element_by_css_selector(
                    "select#equip > option[value='" + EQUIPMENT_TYPE + "']"
                ).click()
            ...
            print(
                "You have 15 minutes to complete this reservation in the browser window."
            )
